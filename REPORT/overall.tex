...
\begin{figure}[H]
  ...
  \caption{The Geometric Warp Drive: Softmax factorization induces massive structural compression.}
...
We swept the success-rate threshold using values $\varepsilon = 10^{-3}$, $10^{-5}$, and $10^{-8}$
across ranks $p \in \{1,\ldots,20\}$ for multiple target
...
Sweeping the Softmax temperature across values $\tau = 0.1, 0.3, 0.5, 1.0, 2.0, 5.0,$ and $10.0$
at fixed precision $\varepsilon = 10^{-5}$, the
